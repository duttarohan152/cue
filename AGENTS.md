...
>
> The speed lever is `output_config.effort` (`low`|`medium`|`high`|`xhigh`|`max`, default `high`), supported on Bedrock, which must sit at the **top level** — putting it inside `thinking` returns a `ValidationException`. A mode sets it via `def.effort`; **only `debug` does** (`medium`), because reading a screenshot line by line is the heaviest thinking cue asks for. Every other mode omits it and keeps `high`. It is forwarded on the **Bedrock path only** — the direct Anthropic provider's default models are Claude 3.5 and would 400 on `output_config`. Fast mode (`speed: "fast"`) is **Claude API only** and unavailable on Bedrock.
>
